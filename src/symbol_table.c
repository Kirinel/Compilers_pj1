...
FUNC_entry *new_func_entry(char *globid, char *ret_type, hash_t hash_code, node *n)
{
  ...
  entry->func_node = n;
  entry->local_vars = init_var_table(VARSPACE);
...
int add_table_func(FUNC_table *t, char *globid, char *ret_type, node *n)
{
  ...
  if (curr == NULL) {
    t->bucs[target_buc] = new_func_entry(globid, ret_type, hash_value, n);
    return 0;
  }
  ...
  curr->next = new_func_entry(globid, ret_type, hash_value, n);
  return 0;
}
...
/* Variable table APIs */
VAR_table *init_var_table(int buckets)
{
  // allocate table memory
...
VAR_entry *new_var_entry(char *name, char *type, hash_t hast_code, node *n)
{
...
int add_table_local_var(VAR_table *t, char *name, char *type, node *n)
{
  ...
  if (curr == NULL) {
    t->bucs[target_buc] = new_var_entry(name, type, hash_value, n);
    return 0;
  }
  ...
  //find the last case and insert;
  curr->next = new_var_entry(name, type, hash_value, n);
  return 0;
}
